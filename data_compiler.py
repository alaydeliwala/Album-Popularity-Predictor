...
def main():
    ...
    for album,artist,year,rank in zip(album_df['album_name'],album_df['artist'],album_df['year'],album_df['rank']):
        if album_features_df.loc[album_features_df['album'] == album].size == 0:
            ...
            # print(output_df.append(temp_df))
            
        # print()
        
    print(match_count)
    # output_df.columns=[['album', 'artist','year','acousticness_mean','acousticness_var','danceability_mean','danceability_var','energy_mean','energy_var','instrumentalness_mean','instrumentalness_var','liveness_mean','liveness_var','loudness_mean','loudness_var','speechiness_mean','speechiness_var','tempo_mean','tempo_var','rank']]
    output_df.to_csv('data/final.csv')
